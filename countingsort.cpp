...
void counting_sort(vector<int> &B, vector<int> &C, int maxvalue)
{
	...
	{
		B[C[A[k]]] = A[k];                 //C벡터(누적값)을 새로운 B벡터에 A벡터 실제값을 넣는다.
		C[A[k]] -= 1;                      //
	}
	return;
}
int main()
{
	A.push_back(3);	A.push_back(6);	A.push_back(4);	A.push_back(1);	A.push_back(3);	A.push_back(4);	A.push_back(1);	A.push_back(4);
